...
// Define GPIO pins for gate relays and indicators (adjust GPIO pins for NodeMCU)
const int gate1Pin = 5;   // GPIO5 (D1)
const int gate2Pin = 4;   // GPIO4 (D2)
const int ledPin = 2;     // GPIO2 (D4)
const int buttonPin = 0;  // GPIO0 (D3)
const int builtInLedPin = LED_BUILTIN; // Use a different GPIO pin for gate2 on ESP8266

// Timing variables for non-blocking relay control
unsigned long gate1OffTime = 0;
unsigned long gate2OffTime = 0;
const unsigned long offDuration = 500; // 5 seconds
...
{ 
  String helloMessage = "Hello Server From ESP8266, connected SSID: " + WiFi.SSID();
  switch (type)
  {
  case WStype_CONNECTED:
    Serial.println("[INFO] Connected to WebSocket server.");
    webSocket.sendTXT(helloMessage);
    // wenbSocket.sendTXT(String("Hello Server From NodeMCU, connected SSID: ") + WiFi.SSID());
...
void loop()
{
  ...
  {
    digitalWrite(gate2Pin, HIGH);
    digitalWrite(builtInLedPin, HIGH);  // LED OFF after the delay
  ...
  // Brief delay to reduce CPU load.
}
